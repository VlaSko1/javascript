...
const map = {
   ...
   * @see {@link https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Array/forEach}
   */
...
const snake = {
  ...
  transitionRowsCount: null, // переход змейки, если он может проходить сквозь стены, строки
  transitionColsCount: null, // переход змейки, если он может проходить сквозь стены, колонки
  ...
  init(startBody, direction, rows, cols) {
    ...
    this.transitionRowsCount = rows - 1;
    this.transitionColsCount = cols - 1;
  },
   ...
   * @see {@link https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Array/some}
    ...
    switch (this.direction) {
      case 'up':
        if (firstPoint.y > 0) {
          ...
          return {x: firstPoint.x, y: this.transitionRowsCount }
        }
      case 'right':
        if (firstPoint.x < this.transitionColsCount) {
          return {x: firstPoint.x + 1, y: firstPoint.y};
        } else {
          return {x: 0, y: firstPoint.y};
        }
      case 'down':
        if (firstPoint.y < this.transitionRowsCount) {
          return {x: firstPoint.x, y: firstPoint.y + 1};
        } else {
      ...
      case 'left':
        if (firstPoint.x > 0) {
          ...
          return {x: this.transitionColsCount, y: firstPoint.y};
        }   
    }
  },
...
const score = {
  ...
  countEl: null,
  /**
   * Метод возвращает текущее значение очков пользователя.
   */
  getCount() {
    return this.count;
  },
  ...
  init() {
    ...
    this.countEl = document.getElementById('scoreGame');
  ...
  render() {
    ...
    this.countEl.textContent = this.getCount();
  },
};
 ...
 * @property {score} score  Объект отображающий счёт игры.
 */
const game = {
  config,
  map,
  ...
  init(userSettings) {
    ...
    // Сбрасываем счетчик игры в ноль.
    this.score.init();
